    ...
    public class MSTSDieselLocomotive : MSTSLocomotive
    {
        ...
        protected override void UpdateMotiveForce(double elapsedClockSeconds, float t, float AbsSpeedMpS, float AbsWheelSpeedMpS)
        {
            ...
            if (PowerOn)
            {
                ...
                {
                    MotiveForceN = (float)TractiveForceCurves.Get(LocomotiveApparentThrottleSetting, AbsSpeedMpS) * (1 - PowerReduction);
        ...
        public override string GetDebugStatus()
        {
            ...
            if (IsSteamHeatFitted && Train.PassengerCarsNumber > 0 && this.IsLeadLocomotive() && Train.CarSteamHeatOn)
            {
                ...
                status.AppendFormat("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6:N0}\t{7}\t{8}\t{9}\t{10}\t{11}\t{12}\t{13}\t{14}\t{15:N0}\n",
        ...
        private void UpdateSteamHeat(double elapsedClockSeconds)
        {
            ...
            if (IsSteamHeatFitted && this.IsLeadLocomotive())  // Only Update steam heating if train and locomotive fitted with steam heating
            {
                ...
                if (CurrentSteamHeatPressurePSI > 0.1 && CurrentSteamHeatBoilerWaterCapacityL > 0 && DieselLevelL > 0 && !IsSteamHeatBoilerLockedOut)      
                {
                    ...
                    CurrentSteamHeatBoilerWaterCapacityL -= (float)(WaterUsageLpS * elapsedClockSeconds); // Reduce Tank capacity as water used.
