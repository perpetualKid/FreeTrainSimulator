...
namespace Orts.Simulation.RollingStocks
{
    ...
    public class MSTSWagon : TrainCar
    {
        ...
        public float WheelRadiusM = (float)Size.Length.FromIn(18.0f);          // provide some defaults in case it's missing from the wag
        ...
        public virtual void LoadFromWagFile(string wagFilePath)
        {
            ...
            if (FrontCouplerShapeFileName != null && !File.Exists(Path.Combine(wagonFolder, FrontCouplerShapeFileName)))
            {
                Trace.TraceWarning("{0} references non-existent shape {1}", WagFilePath, Path.GetFullPath(Path.Combine(wagonFolder, FrontCouplerShapeFileName)));
                FrontCouplerShapeFileName = null;
            }

            if (RearCouplerShapeFileName != null && !File.Exists(Path.Combine(wagonFolder, RearCouplerShapeFileName)))
            {
                Trace.TraceWarning("{0} references non-existent shape {1}", WagFilePath, Path.GetFullPath(Path.Combine(wagonFolder, RearCouplerShapeFileName)));
       ...
       private void UpdateLocomotiveLoadPhysics()
        ...
        private void UpdateTrainBaseResistance_DavisLowSpeed()
        {
            ...
            if (IsRollerBearing)
            {
                ...
                float LowTemperatureResistance = (float)Dynamics.Force.FromLbf( 12.0f ) * wheelvariationfactor;
                float HighTemperatureResistance = (float)Dynamics.Force.FromLbf( 4.5f) * wheelvariationfactor;
            ...
            else if (IsLowTorqueRollerBearing)
            {
                ...
                float LowTemperature = -10.0f;
                float HighTemeprature = 25.0f;
                float LowTemperatureResistance = (float)Dynamics.Force.FromLbf(7.5f) * wheelvariationfactor;
                float HighTemperatureResistance = (float)Dynamics.Force.FromLbf(2.5f) * wheelvariationfactor;
            ...
            else if (IsGreaseFrictionBearing)
            {
                ...
                float LowTemperature = -10.0f;
                float HighTemeprature = 25.0f;
                float LowTemperatureResistance = (float)Dynamics.Force.FromLbf(45.0f) * wheelvariationfactor;
                float HighTemperatureResistance = (float)Dynamics.Force.FromLbf(30.0f) * wheelvariationfactor;
            ...
            else  // default to friction (solid - oil journal) bearing
            {
                ...
                float LowTemperature = -10.0f;
                float HighTemeprature = 25.0f;
                float LowTemperatureResistance = (float)Dynamics.Force.FromLbf(30.0f) * wheelvariationfactor;
                float HighTemperatureResistance = (float)Dynamics.Force.FromLbf(20.0f) * wheelvariationfactor;
                ...
                {
                    // Set to snowing (frozen value)
                    StartFrictionInternalFactorN = (float)Dynamics.Force.FromLbf(LowTemperatureResistance); // Starting friction for car with friction (journal) bearings - ton (US), snowing
                }
                else if (WheelBearingTemperatureDegC > 25)
                {
                    // Set to normal temperature value
                    StartFrictionInternalFactorN = (float)Dynamics.Force.FromLbf(HighTemperatureResistance); // Starting friction for car with friction (journal) bearings - ton (US), not snowing
                }
                else
                {
                    // Set to variable value as bearing heats and cools
