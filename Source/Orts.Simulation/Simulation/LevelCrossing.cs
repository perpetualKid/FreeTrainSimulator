...
, (KeyValuePair<int, LevelCrossingItem> _) => _.Key, (KeyValuePair<int, LevelCrossingItem> _) => _.Value);
        ...
        void UpdateCrossings(Train train, double elapsedTime)
        {
            ...
            foreach (var crossing in TrackCrossingItems.Values.Where(ci => ci.CrossingGroup != null))
            {
                ...
                // Train is stopped.
                else if ((train is AITrain || train.TrainType == Train.TRAINTYPE.PLAYER || train.TrainType == Train.TRAINTYPE.REMOTE) && speedMpS == 0 && frontDist <= reqDist && (train.ReservedTrackLengthM <= 0 || frontDist < train.ReservedTrackLengthM) && rearDist <= minimumDist)
                {
