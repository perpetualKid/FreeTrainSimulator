...
namespace Orts.Simulation.Timetables
{
    public class TimetableInfo
    {
        ...
        private List<TTTrain> BuildAITrains(Collection<TTTrainInfo> allTrains, TTTrainInfo playerTrain, out bool allPathsLoaded)
        {
            ...
            foreach (TTTrainInfo reqTrain in allTrains)
            {
                ...
                if (trainRouteCrossRef.ContainsKey(reqTrain.Index) && Paths.TryGetValue(trainRouteCrossRef[reqTrain.Index], out AIPath value))
                {
                    ...
                    reqTrain.TTTrain.RearTrackTraveller = Train.InitializeDirectedTraveller(usedPath.FirstNode.Location, usedPath.FirstNode.NextMainNode.Location);
        ...
        private void PreInitPlayerTrain(TTTrainInfo reqTrain)
        {
            ...
            playerTrain.RearTrackTraveller = Train.InitializeDirectedTraveller(usedPath.FirstNode.Location, usedPath.FirstNode.NextMainNode.Location);
        ...
        private class TTTrainInfo
        {
            ...
            public bool ProcessDisposeInfo(ref List<TTTrain> trainList, TTTrainInfo playerTrain, Simulator simulator)
            {
                ...
                if (DisposeDetails.Stable && (trainFound || DisposeDetails.FormStatic))
                {
                    ...
                    {
                        outTrain.RearTrackTraveller = Train.InitializeDirectedTraveller(outPath.FirstNode.Location, outPath.FirstNode.NextMainNode.Location);
                    ...
                    else
                    {
                        ...
                        {
                            inTrain.RearTrackTraveller = Train.InitializeDirectedTraveller(inPath.FirstNode.Location, inPath.FirstNode.NextMainNode.Location);
                            inTrain.Path = inPath;
            ...
            public bool BuildRunRound(ref TTTrain rrtrain, bool atStart, DisposeInfo disposeDetails, Simulator simulator, ref List<TTTrain> trainList)
            {
                ...
                {
                    formedTrain.RearTrackTraveller = Train.InitializeDirectedTraveller(formedPath.FirstNode.Location, formedPath.FirstNode.NextMainNode.Location);
