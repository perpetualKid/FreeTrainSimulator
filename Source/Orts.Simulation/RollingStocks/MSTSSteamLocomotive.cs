...
namespace Orts.Simulation.RollingStocks
{
    ...
    public class MSTSSteamLocomotive : MSTSLocomotive
    {
        ...
        private float LPCompPressure_g_AtmPSI;
        private float LPCompPressure_l_AtmPSI;
        private float LPCompPressure_h_AtmPSI;
        private float LPCompPressure_m_AtmPSI;
        private float LPCompPressure_n_AtmPSI;
        private float LPCompPressure_q_AtmPSI;
        ...
        private float RodCoGFt = 4.32f;  // 0.4 from crank end of rod
        private float DisplayTangentialWheelTreadForceLbf;
        #endregion
        ...
        private void UpdateFX(double elapsedClockSeconds)
        {
            ...
            if (CylinderAdvancedSteamExhaustEffects)
            {
                ...
                if (CylinderSteamExhaust1On || CylinderSteamExhaust2On || CylinderSteamExhaust3On || CylinderSteamExhaust4On)
                {
                    ...
                    StackSteamVelocityMpS.Update(elapsedClockSeconds, (float)Math.Sqrt(Pressure.Standard.FromPSI(Pressure_c_AtmPSI) * 1000 * smokeVelocityVariationFactor / WaterDensityAt100DegC1BarKGpM3));
                    StackSteamVolumeM3pS = (float)Mass.Kilogram.FromLb(CylinderSteamUsageLBpS + BlowerSteamUsageLBpS + RadiationSteamLossLBpS + CompSteamUsageLBpS + GeneratorSteamUsageLBpS) * smokeVolumeVariationFactor * SteamVaporSpecVolumeAt100DegC1BarM3pKG;
                ...
                else // when not exhausting
                {
                    if (absSpeedMpS < 10)
                    {
                        ...
                        StackSteamVolumeM3pS = (float)Mass.Kilogram.FromLb(BlowerSteamUsageLBpS + RadiationSteamLossLBpS + CompSteamUsageLBpS + GeneratorSteamUsageLBpS) * smokeRestVolumeVariationFactor * SteamVaporSpecVolumeAt100DegC1BarM3pKG;
            ...
            {
                float velocityRate = (float)Math.Sqrt(Pressure.Standard.FromPSI(Pressure_c_AtmPSI) * 1000 * 2 / WaterDensityAt100DegC1BarKGpM3);
        ...
        private void UpdateCylinders(double elapsedClockSeconds, float throttle, float cutoff, float absSpeedMpS)
        {
            ...
            if (SteamEngineType == SteamEngineType.Compound)
            {
                ...
                else
                {
                    ...
                    LPCompPressure_m_AtmPSI = (float)(BackPressureIHPtoPSI[IndicatedHorsePowerHP] + Const.OneAtmospherePSI);
        ...
        protected override void UpdateTractiveForce(double elapsedClockSeconds, float t, float AbsSpeedMpS, float AbsWheelSpeedMpS)
        {
            ...
            TractiveForceN = (float)Dynamics.Force.FromLbf(TractiveEffortLbsF);
            ...
            {
                TractiveForceN = (float)Dynamics.Force.FromLbf((MaxIndicatedHorsePowerHP * 375.0f) / Frequency.Periodic.ToHours(Size.Length.ToMi(SpeedMpS)));
            ...
            DrawBarPullLbsF = (float)Dynamics.Force.ToLbf(Math.Abs(TractiveForceN) - LocoTenderFrictionForceN); // Locomotive drawbar pull is equal to motive force of locomotive (+ tender) - friction forces of locomotive (+ tender)
            ...
            // Don't use slip model when train is in auto pilot
            {
                ...
                for (int i = 0; i < NumCylinders; i++)
                {
                    ...
                    */
                    ...
                    float pistonForceLbf = (float)Size.Area.ToIn2(Size.Area.FromFt2(CylinderPistonAreaFt2)) * crankCylinderPressure;
                    ...
                    float inertiaSpeedCorrectionFactor = 0.00034f * CrankRadiusFt * (float)Math.Pow(Frequency.Periodic.ToMinutes(DrvWheelRevRpS), 2);
                    ...
                    float tangentialWheelTreadForceLbf = (float)(tangentialCrankWheelForceLbf * Size.Length.ToIn(CylinderStrokeM) / Size.Length.ToIn(DrvWheelDiaM));
                    DisplayTangentialWheelTreadForceLbf += tangentialWheelTreadForceLbf;
                    TractiveForceN += (float)Dynamics.Force.FromLbf(Math.Max(tangentialWheelTreadForceLbf, -1000));
                    ...
                    float excessBalanceWeightLb = (ConnectingRodWeightLb + ReciprocatingWeightLb) - ConnectingRodBalanceWeightLb - ((float)Mass.Kilogram.ToLb(MassKG) / ExcessBalanceFactor);
                    ...
                    {
                        totalDrvWeightN += (float)Dynamics.Force.FromLbf(excessBalanceForcelbf + verticalThrustForcelbf);
                    }
                    else
                    {
                        totalDrvWeightN += (float)Dynamics.Force.FromLbf(excessBalanceForcelbf - verticalThrustForcelbf);
                    ...
                    /*                    if (DisplayTangentialWheelTreadForceLbf > SteamStaticWheelForce || WheelSlip)
                                        {
                                            ...
                                            Trace.TraceInformation("Crank Delta {0} Cylinder Position {1} AxlePosition {2} Cylinder {3} CylArea {4} CylPress {5}", MathHelper.ToDegrees(crankAngleRad), crankCylinderPosition, MathHelper.ToDegrees(LocomotiveAxle.AxlePositionRad), i, Me2.ToIn2(Me2.FromFt2(CylinderPistonAreaFt2)), crankCylinderPressure);
                ...
                SteamStaticWheelForce = (float)Dynamics.Force.ToLbf(totalDrvWeightN + 9.81f * DrvWheelWeightKg) * LocomotiveCoefficientFrictionHUD;
                /*
        ...
        public override void AdvancedAdhesion(double elapsedClockSeconds)
        {
            if (SteamEngineType != SteamEngineType.Geared)
            {
                ...
                LocomotiveAxle.FrictionN = (float)(Dynamics.Force.FromLbf(3.8f * Size.Length.ToIn(CylinderDiameterM) * Size.Length.ToIn(CylinderDiameterM) * Size.Length.ToIn(CylinderStrokeM) / Size.Length.ToIn(DrvWheelDiaM)));
