...
namespace Orts.Simulation.World
{
    public class LevelCrossings
    {
        ...
        public LevelCrossings()
        {
            ...
            TrackDatabase roadDatabase = RuntimeDataResolver.Instance?.TrackWorld.RoadDatabase;
        ...
        /// <summary>
        /// Computes the distance from a crossing to a train end using the
        /// <see cref="TrackTraveller"/>.
        /// </summary>
        private static float DistanceToWithTrace(LevelCrossingItem crossing, TrackTraveller traveller, float maxDistance, bool reversed = false)
        {
            TrackTraveller effective = reversed ? traveller.Reverse() : traveller;
            return crossing.DistanceTo(effective, maxDistance);
        }
        ...
        public LevelCrossingItem SearchNearLevelCrossing(Train train, float reqDist, bool trainForwards, out float frontDist)
        {
            ...
            foreach (LevelCrossingItem crossing in TrackCrossingItems.Values.Where(ci => ci.CrossingGroup != null))
                ...
                {
                    frontDist = DistanceToWithTrace(crossing, traveller, reqDist, reversed);
