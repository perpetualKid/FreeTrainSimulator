    ...
    public class Train
    {
            ...
            ref EndAuthorityType endAuthority, ref float endAuthorityDistanceM)
        {
            ...
            if (!endWithSignal && totalLengthM < minCheckDistanceManualM)
            {
                ...
                int nextSectionIndex = lastSection.Pins[(TrackDirection)lastElement.OutPin[0], (Location)lastElement.OutPin[1]].Link;
                TrackDirection nextSectionDirection = lastSection.Pins[(TrackDirection)lastElement.OutPin[0], (Location)lastElement.OutPin[1]].Direction;
        ...
        public void ProcessExplorerSwitch(int routeDirectionIndex, TrackCircuitSection switchSection, Direction direction)
        {
            ...
            if (firstSignal != null)
            {
                ...
                //<CSComment> Probably also in singleplayer the logic of multiplayer should be used, but it's unwise to modify it just before a release
                if (switchSection.Pins[TrackDirection.Ahead, Location.NearEnd].Link == selectedRoute[lastIndex].TCSectionIndex || !MPManager.IsMultiPlayer())
        ...
        public void GenerateValidRoute(int forcedRouteSectionIndex, int forcedTCSectionIndex)
        {
            ...
            if (tempSections.Count > 0)
            {
                ...
                foreach (int sectionIndex in tempSections)
                {
                    ...
                    thisElement = new TCRouteElement(signalRef.TrackCircuitList[Math.Abs(sectionIndex)],
                            (int)sectionDirection, signalRef, prevSection);
        ...
        public TrainInfo GetTrainInfoAuto()
        {
            ...
            {
                result.ObjectInfoForward.Add(new TrainObjectItem(EndAuthorityTypes[0], DistanceToEndNodeAuthorityM[0]));
                maxAuthSet = true;
            }
            ...
            foreach (SignalItemInfo signalItem in SignalObjectItems)
            {
                if (signalItem.ItemType == SignalItemType.Signal)
                {
                    ...
                    if (signalItem.SignalDetails.EnabledTrain == null || signalItem.SignalDetails.EnabledTrain.Train != this)
                    {
                        ...
                        result.ObjectInfoForward.Add(new TrainObjectItem(signalAspect, signalItem.ActualSpeed, signalItem.DistanceToTrain));
                ...
                {
                    result.ObjectInfoForward.Add(new TrainObjectItem(signalItem.ActualSpeed, signalItem.DistanceToTrain, (TrainObjectItem.SpeedItemType)(signalItem.SpeedInfo.LimitedSpeedReduction)));
                }
            }
            ...
            if (!signalProcessed && NextSignalObject[0]?.EnabledTrain?.Train == this)
            {
                ...
                result.ObjectInfoForward.Add(new TrainObjectItem(signalAspect, validSpeed, DistanceToSignal));
            }
            ...
            {
                result.ObjectInfoForward.Add(new TrainObjectItem(StationStops[0].DistanceToTrainM, (int)StationStops[0].PlatformItem.Length));
            }
            ...
            {
                result.ObjectInfoBackward.Add(new TrainObjectItem(EndAuthorityType.NoPathReserved, 0.0f));
            }
            else
            {
                if (RearSignalObject != null)
                {
                    ...
                    result.ObjectInfoBackward.Add(new TrainObjectItem(signalAspect, -1.0f, ClearanceAtRearM));
                }
                else
                {
                    result.ObjectInfoBackward.Add(new TrainObjectItem(EndAuthorityType.EndOfAuthority, ClearanceAtRearM));
                }
            }
            return result;
        }
                    ...
                    for (int iRouteElement = startRouteIndex; iRouteElement < ValidRoute[routeDirection].Count && distanceToTrainM < 7000 && sectionStart < 7000; iRouteElement++)
                    {
                        ...
                        if (thisSection.CircuitItems.TrackCircuitMileposts != null)
                        {
                            foreach (TrackCircuitMilepost thisMilepostItem in thisSection.CircuitItems.TrackCircuitMileposts)
                            {
                                ...
                                {
                                    thisItem = new TrainObjectItem(thisMilepost.Value.ToString(), distanceToTrainM);
        ...
        internal virtual void AddTrainReversalInfo(TrainInfo trainInfo, TCReversalInfo reversalInfo)
        {
            ...
            {
                trainInfo.ObjectInfoForward.Add(new TrainObjectItem(reversalEnabled, reversalDistanceM, reversalInfo.Valid));
            }
        }
        ...
        internal void AddWaitingPointInfo(TrainInfo trainInfo)
        {
                ...
                (AuxActionsContain.SpecAuxActions[0] as AIActionWPRef).SubrouteIndex == TCRoute.activeSubpath)
            {
                ...
                trainInfo.ObjectInfoForward.Add(new TrainObjectItem(wpDistance, wpEnabled));
            }
        }
        ...
        private TrainInfo GetTrainInfoManual()
        {
            ...
            if (ValidRoute[0] != null)
            {
                ...
                foreach (TCRouteElement thisElement in ValidRoute[0])
                {
                    ...
                    if (thisSection.EndSignals[sectionDirection] != null)
                    {
                        ...
                        result.ObjectInfoForward.Add(new TrainObjectItem(signalAspect, validSpeed, distanceToTrainM));
                    }
                    ...
                    if (thisSection.CircuitItems.TrackCircuitSpeedPosts[sectionDirection] != null)
                    {
                        foreach (TrackCircuitSignalItem thisSpeeditem in thisSection.CircuitItems.TrackCircuitSpeedPosts[sectionDirection])
                        {
                            ...
                            if (distanceToTrainM > 0 && (validSpeed > 0 || (thisSpeedInfo != null && thisSpeedInfo.Reset)))
                            {
                                ...
                                result.ObjectInfoForward.Add(new TrainObjectItem(validSpeed, distanceToTrainM, (TrainObjectItem.SpeedItemType)thisSpeedpost.SpeedPostType()));
                            }
                        }
                    }
            ...
            // set authority
            result.ObjectInfoBackward.Add(new TrainObjectItem(EndAuthorityTypes[1], DistanceToEndNodeAuthorityM[1]));
            ...
            if (ValidRoute[1] != null)
            {
                ...
                foreach (TCRouteElement thisElement in ValidRoute[1])
                {
                    ...
                    if (thisSection.EndSignals[sectionDirection] != null)
                    {
                        ...
                        TrackMonitorSignalAspect signalAspect = thisSignal.TranslateTMAspect(thisSignal.SignalLR(SignalFunction.Normal));
                        result.ObjectInfoBackward.Add(new TrainObjectItem(signalAspect, validSpeed, distanceToTrainM));
                    }
                    ...
                    if (thisSection.CircuitItems.TrackCircuitSpeedPosts[sectionDirection] != null)
                    {
                        foreach (TrackCircuitSignalItem thisSpeeditem in thisSection.CircuitItems.TrackCircuitSpeedPosts[sectionDirection])
                        {
                            ...
                            if (distanceToTrainM > 0 && (validSpeed > 0 || (thisSpeedInfo != null && thisSpeedInfo.Reset)))
                            {
                                ...
                                result.ObjectInfoBackward.Add(new TrainObjectItem(validSpeed, distanceToTrainM, (TrainObjectItem.SpeedItemType)thisSpeedpost.SpeedPostType()));
                            }
                        }
                    }
        ...
        public TrainInfo GetTrainInfoOOC()
        {
            ...
            result.ObjectInfoForward.Add(new TrainObjectItem(OutOfControlReason));
            return result;
        }
        ...
        public class TCRoutePath
        {
            ...
            public TCRoutePath(AIPath aiPath, int orgDir, float thisTrainLength, SignalEnvironment orgSignals, int trainNumber, UserSettings settings)
            {
                ...
                if (lastEndSection.CircuitType != TrackCircuitType.EndOfTrack && lastEndSection.EndSignals[lastDirection] == null)
                {
                    ...
                    {
                        addedElements.Add(new TCRouteElement(lastEndSection.Index, (int)lastDirection));
                    ...
                    if (lastEndSection.CircuitType == TrackCircuitType.EndOfTrack)
                    {
                        ...
                        thisSubpath.Add(new TCRouteElement(lastEndSection.Index, (int)lastDirection));
                    }
                }
                ...
                foreach (TCSubpathRoute endSubPath in TCRouteSubpaths)
                {
                    ...
                    int nextSectionIndex = lastSection.ActivePins[(TrackDirection)thisElement.OutPin[0], (Location)thisElement.OutPin[1]].Link;
                    TrackDirection nextDirection = lastSection.ActivePins[(TrackDirection)thisElement.OutPin[0], (Location)thisElement.OutPin[1]].Direction;
        ...
        public class TCRouteElement
        {
            ...
            public TCRouteElement(TrackNode thisNode, int TCIndex, int direction, SignalEnvironment mySignals)
            {
                ...
                if (thisSection.CircuitType == TrackCircuitType.Crossover)
                {
                    TrackDirection outPinLink = (TrackDirection)direction;
                    ...
                    OutPin[1] = (thisSection.Pins[outPinLink, Location.NearEnd].Link == nextIndex) ? 0 : 1;
                }

                FacingPoint = false;
                if (thisSection.CircuitType == TrackCircuitType.Junction)
                {
                    if (thisSection.Pins[(TrackDirection)direction, Location.FarEnd].Link != -1)
                    {
                        FacingPoint = true;
                    }
                }
            ...
            public TCRouteElement(TrackCircuitSection thisSection, int direction, SignalEnvironment mySignals, int lastSectionIndex)
            {
                ...
                if (thisSection.CircuitType == TrackCircuitType.Crossover)
                {
                    TrackDirection inPinLink = direction == 0 ? TrackDirection.Reverse : TrackDirection.Ahead;
                    OutPin[1] = (thisSection.Pins[inPinLink, Location.NearEnd].Link == lastSectionIndex) ? 0 : 1;
                }
                    ...
                    if (thisSection.CircuitType == TrackCircuitType.Junction)
                    {
                        if (newElement.FacingPoint)
                        {
                            if (iIndex >= 1)
                            {
                                ...
                                newElement.OutPin[1] = (thisSection.Pins[TrackDirection.Reverse, Location.NearEnd].Link == this[iIndex - 1].TCSectionIndex) ? 0 : 1;
                            }
                        }
                        else
                        {
