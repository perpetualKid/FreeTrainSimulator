...
#pragma warning restore CA1002 // Do not expose generic lists
        ...
        public async ValueTask<SimulatorSaveState> Snapshot()
        {
            ...
            return new SimulatorSaveState()
            {
                ...
                TimetableTrains = TimetableMode ? await new Collection<Train> { Trains[0] }.Concat(AI.AITrains).SnapshotCollection<TrainSaveState, Train>().ConfigureAwait(false) : 
                    await AI.AITrains.SnapshotCollection<TrainSaveState, Train>().ConfigureAwait(false),
