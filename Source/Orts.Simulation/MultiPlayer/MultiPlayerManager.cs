...
//				Thread.Sleep(1000);
		...
		public void AddPlayer()
		{
			...
			{
				PlayerAdded = false;
				MultiPlayerManager.Instance().lastPlayerAddedTime = Simulator.GameTime;
				MultiPlayerManager.Instance().lastSwitchTime = Simulator.GameTime;

				MSGPlayer host = new  MSGPlayer(MultiPlayerManager.GetUserName(), "1234", Simulator.ConsistFileName, Simulator.PathFileName, Simulator.PlayerLocomotive.Train,
					Simulator.PlayerLocomotive.Train.Number, Simulator.Settings.AvatarURL);
				MultiPlayerManager.BroadCast(host.ToString() + MultiPlayerManager.OnlineTrains.AddAllPlayerTrain());
				foreach (Train t in Simulator.Trains)
				{
					...
					MultiPlayerManager.BroadCast((new MSGTrain(t, t.Number)).ToString());
				}
				if (CheckSpad == false) { MultiPlayer.MultiPlayerManager.BroadCast((new MultiPlayer.MSGMessage("All", "OverSpeedOK", "OK to go overspeed and pass stop light")).ToString()); }
                ...
                MultiPlayerManager.BroadCast(GetEnvInfo());
			}
		}
		...
		public string GetOnlineUsersInfo()
		{
			...
			try//the list of players may be changed during the following process
			{
				...
				foreach (OnlinePlayer p in OnlineTrains.Players.Values)
				{
					...
					users.Add(Math.Sqrt(d)+MultiPlayerManager.Random.NextDouble(), p.Username);
				}
			}
			catch (Exception)
			{
        ...
        public TrainCar SubCar(string wagonFilePath, int length)
		{
			...
			try
			{
				...
				if (Simulator.Confirmer != null)
                    Simulator.Confirmer.Information(MultiPlayerManager.Catalog.GetString("Missing car, have substituted with other one."));
