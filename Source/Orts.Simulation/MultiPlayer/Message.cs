...
namespace Orts.Simulation.MultiPlayer
{
    ...
    public class MSGPlayer : MSGRequired
    {
        ...
        public override void HandleMsg()
        {
            ...
            lock (lockObjPlayer)
            {
                ...
                if (MultiPlayerManager.FindPlayerTrain(user) != null) return; //already added the player, ignore
    ...
    public class MSGTrain : Message
    {
        ...
        public override void HandleMsg() //only client will get message, thus will set states
        {
            ...
            train.RearTDBTraveller = new Traveller(Simulator.Instance.TrackDatabase.TrackDB.TrackNodes, location, direction == 1 ? Direction.Forward : Direction.Backward);
            ...
            for (var i = 0; i < cars.Length; i++)// cars.Length-1; i >= 0; i--) {
            {
                ...
                catch (Exception error)
                {
                    ...
                    car = MultiPlayerManager.Instance().SubCar(wagonFilePath, lengths[i]);
                }
            ...
            if (MultiPlayerManager.Instance().AddOrRemoveTrain(train, true) == false) return; //add train, but failed
    ...
    public class MSGUpdateTrain : Message
    {
        ...
        {
            if (MultiPlayerManager.IsServer()) return; //server will ignore it
            if (user != MultiPlayerManager.GetUserName()) return; //not the one requested GetTrain
            ...
            if (found)
            {
                Traveller traveller = new Traveller(Simulator.Instance.TrackDatabase.TrackDB.TrackNodes, location, direction == 1 ? Direction.Forward : Direction.Backward);
            ...
            train1.RearTDBTraveller = new Traveller(Simulator.Instance.TrackDatabase.TrackDB.TrackNodes, location, direction == 1 ? Direction.Forward : Direction.Backward);
            for (var i = 0; i < cars.Length; i++)// cars.Length-1; i >= 0; i--) {
            {
                string wagonFilePath = Path.Combine(Simulator.Instance.RouteFolder.ContentFolder.TrainSetsFolder, cars[i]);
                TrainCar car = null;
            ...
            MultiPlayerManager.Instance().AddOrRemoveTrain(train1, true);
            if (MultiPlayerManager.IsServer()) MultiPlayerManager.Instance().AddOrRemoveLocomotives(user, train1, true);
        }
        ...
        public override void HandleMsg()
        {
            foreach (int i in trains)
            {
                foreach (Train train in Simulator.Instance.Trains)
                {
                    if (i == train.Number)
                    {
                        if (MultiPlayerManager.IsServer()) MultiPlayerManager.Instance().AddOrRemoveLocomotives("", train, false);
            ...
            foreach (var t in Simulator.Instance.Trains)
            {
                foreach (var car in t.Cars)
                {
                    if (car.CarID == engine)
                    {
                        ...
                        if (MultiPlayerManager.IsServer()) MultiPlayerManager.BroadCast((new MSGLocoChange(user, engine, frontOrRearCab, t)).ToString());
                        return;
                    }
                }
            }
            ...
            else
            {
                ...
                foreach (Train t in Simulator.Instance.Trains)
                {
                    ...
                    if (found == true)
                    {
                        ...
                        t.RearTDBTraveller = new Traveller(Simulator.Instance.TrackDatabase.TrackDB.TrackNodes, location1, d1);
                ...
                train2.RearTDBTraveller = new Traveller(Simulator.Instance.TrackDatabase.TrackDB.TrackNodes, location2, d2);
    ...
    public class MSGCouple : Message
    {
        ...
        public override void HandleMsg()
        {
            ...
            train.DistanceTravelled = Travelled;
            train.MUDirection = TranslateMidpointDirection(mDirection); // (MidpointDirection)mDirection;
            train.RearTDBTraveller = new Traveller(Simulator.Instance.TrackDatabase.TrackDB.TrackNodes, location, direction == 0 ? Direction.Forward : Direction.Backward);
            train.CheckFreight();
            train.CalculatePositionOfCars();
    ...
    public class MSGText : MSGRequired
    {
        ...
        public override void HandleMsg()
        {
            if (sender == MultiPlayerManager.GetUserName()) return; //avoid my own msg
            ...
            if (MultiPlayerManager.IsServer())//server check if need to tell others.
            {
                ...
                if (users.Length == 1 && users[0].Trim() == MultiPlayerManager.GetUserName()) return;
