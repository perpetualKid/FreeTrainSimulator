...
namespace MSTS.Formats
{
    ...
    public class TrackDB
    {
        ...
        public void AddRestrictZone(Tr_RouteFile routeFile, TSectionDatFile TSection, ActivityRestrictedSpeedZones zone)
        {
            ...
            {
                tmpTable[idx] = new SpeedPostItem(routeFile, z.StartPosition, idx, true);
                ModifyItems(z.StartPosition, TSection, (int)idx);
                idx++;
                tmpTable[idx] = new SpeedPostItem(routeFile, z.EndPosition, idx, false);
	...
	public int DisplayNumber;   // number to be displayed if ShowNumber is true
        ...
        public SpeedPostItem(Tr_RouteFile routeFile, Position s, uint idx, bool start)
        {
            ItemType = trItemType.trSPEEDPOST;
            TrItemId = idx;
            CreateRPData(s);
            ...
            var speed = routeFile.TempRestrictedSpeed;
            if (speed < 0) speed = 6.9444f;
            if (routeFile.MilepostUnitsMetric == true)
            {
                this.IsMPH = false;
                SpeedInd = (int)(routeFile.TempRestrictedSpeed * 3.6f) + 1;
            }
            else
            {
                this.IsMPH = true;
                SpeedInd = (int)(routeFile.TempRestrictedSpeed * 2.236936f) + 1;
            }
