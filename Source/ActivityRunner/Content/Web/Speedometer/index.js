...
function updateDisplays() {
    ...
    var speed = $('#TrainDriving td:contains("Speed")').parent().children()[3].innerHTML.replace(/[^0-9\.]+/g,"");
    var limit = $('#TrackMonitor td:contains("Limit")').parent().children()[3].innerHTML.replace(/[^0-9\.]+/g,"");
    console.log("Found speed: " + speed);
    console.log("Found speed limit: " + limit);
    
    speedometer.attr('data-value',speed);
    speedometer.attr('data-highlights', '[{ "from": 0, "to": ' + limit + ', "color": "#666" }]');
    
    if ((Number(speed) - Number(limit)) > 2) {
        // overspeed
        speedometer.attr('data-color-needle', '#f00');
        speedometer.attr('data-color-needle-end', "#f00");
    } else {
        speedometer.attr('data-color-needle', '#fff');
        speedometer.attr('data-color-needle-end', "#fff");
    }
}
