...
namespace Orts.ActivityRunner.Viewer3D.Popups
{
    public class EOTListWindow : Window
    {
        ...
        protected override ControlLayout Layout(ControlLayout layout)
        {
            ...
            if (Owner.Viewer.Simulator.FullEOTPaths != null)
            {
                ...
                foreach (string eotType in Owner.Viewer.Simulator.FullEOTPaths)
                {
                    ...
                    if (playerLocomotive?.Train != null && eotType.Equals(playerLocomotive.Train.EndOfTrainDevice?.WagFilePath, StringComparison.OrdinalIgnoreCase)) 
                    {                       
    ...
    class EOTLabel : Label
    {
        ...
        private readonly string pickedEOTTypeFromList;
        ...
        {
            if (!string.IsNullOrEmpty(pickedEOTTypeFromList))
            {
                ...
                if (!pickedEOTTypeFromList.Equals(viewer.PlayerLocomotive.Train.Cars[^1].WagFilePath, StringComparison.OrdinalIgnoreCase))
                {
                    if (viewer.PlayerLocomotive.Train?.EndOfTrainDevice != null)
                    {
                        viewer.Simulator.Confirmer.Information(Viewer.Catalog.GetString("Player train already has a mounted EOT"));
                        return;
                    }
                    //Ask to mount EOT
                    _ = new EOTMountCommand(viewer.Log, true, pickedEOTTypeFromList);
                }
                else if (pickedEOTTypeFromList.Equals(viewer.PlayerLocomotive.Train.Cars[^1].WagFilePath, StringComparison.OrdinalIgnoreCase))
                {
                    _ = new EOTMountCommand(viewer.Log, false, pickedEOTTypeFromList);
                }
                else
                {
