    ...
    internal sealed class GraphicInformation : DetailInfoBase
    {
        ...
        public override void Update(GameTime gameTime)
        {
            if (UpdateNeeded)
            {
                ...
                if (viewer.UserSettings.DynamicShadows)
                {
                    this["Shadow Maps"] = string.Join('\t', new string[] { $"({viewer.UserSettings.ShadowMapResolution}x{viewer.UserSettings.ShadowMapResolution})" }.Concat(Enumerable.Range(0, RenderProcess.ShadowMapCount).Select(i => $"{RenderProcess.ShadowMapDistance[i]}/{RenderProcess.ShadowMapDiameter[i]}")));
