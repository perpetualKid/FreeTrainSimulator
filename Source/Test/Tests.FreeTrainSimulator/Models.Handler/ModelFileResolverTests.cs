...
namespace Tests.FreeTrainSimulator.Models.Handler
{
    ...
    public class ModelFileResolverTests
    {
        ...
        {
            CurrentProfileSettingsModel current = new CurrentProfileSettingsModel
            {
                Profile = "Something"
            };
            string targetFileName = ModelFileResolver<CurrentProfileSettingsModel>.FilePath(current);
            Assert.IsTrue(targetFileName.EndsWith($"{ModelFileResolver<CurrentProfileSettingsModel>.SubFolder}\\{ModelFileResolver<CurrentProfileSettingsModel>.FileExtension}", System.StringComparison.OrdinalIgnoreCase));
        }
