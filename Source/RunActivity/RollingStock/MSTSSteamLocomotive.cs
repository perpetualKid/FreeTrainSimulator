...
namespace ORTS
{
  ...
  #region Additional steam properties
        ...
        float CylCockSteamUsageLBpS = 0.0f; // Cylinder Cock Steam Usage
        ...
        float BoilerFeedwaterHeatFactor = 3.0f;     // factor to increase the boiler feedwater temperature (via injector) to lessen impact of adding water to boiler
        ...
        float Injector1StartCapLBspS;   // Starting value for injector 1 delivery temp - ie minimum delivery temp
        float Injector2StartCapLBspS;   // Starting value for injector 2 delivery temp - ie minimum delivery temp
        ...
        private void UpdateFirebox(float elapsedClockSeconds, float absSpeedMpS)
        {
            ...
            else
            {
                ...
                // BurnRateRawLBpS = Kg.ToLb(BurnRateLBpStoKGpS[(HeatRatio * pS.FrompH(MaxBoilerOutputLBpH))]); // Original
                BurnRateRawLBpS = Kg.ToLb(BurnRateLBpStoKGpS[(HeatRatio * PreviousTotalSteamUsageLBpS)]); // Original
        ...
        private void UpdateCylinders(float elapsedClockSeconds, float throttle, float cutoff, float absSpeedMpS)
        {
            ...
            CylinderPressurePSI = throttle * BoilerPressurePSI - CylinderPressureDropLBpStoPSI[CylinderSteamUsageLBpS + CylCockSteamUsageLBpS];
        ...
        {
            MotiveForceN = (Direction == Direction.Forward ? 1 : -1) * (BackPressurePSI * ForceFactor1NpPSI[cutoff] + CylinderPressurePSI * ForceFactor2NpPSI[cutoff]); // Original Formula
            // Derate when needed.
            if (CylinderCocksAreOpen)
                MotiveForceN -= MotiveForceN * CylinderCocksDeratingFactor;

            if (BoilerIsPriming)
                MotiveForceN -= MotiveForceN * BoilerPrimingDeratingFactor;
        ...
        private void UpdateInjectors(float elapsedClockSeconds)
        {
            ...
            else
            {
                ...
                if (Injector1IsOn)
                {
                    ...
                    {
                    Injector1TempFraction = (Injector1Fraction - InjCapMinFactorX[BoilerPressurePSI]) / (1 - InjCapMinFactorX[PSI]); // Find the fraction above minimum value
                    ...
                    BoilerHeatBTU -= elapsedClockSeconds * Injector1Fraction * InjectorFlowRateLBpS * (BoilerWaterHeatBTUpLB - WaterHeatPSItoBTUpLB[Injector1WaterTempPressurePSI]); // Loss of boiler heat due to water injection - loss is the diff between steam and water Heat   
                    ...
                    BoilerHeatOutBTUpS += Injector1Fraction * InjectorFlowRateLBpS * (BoilerWaterHeatBTUpLB - WaterHeatPSItoBTUpLB[Injector1WaterTempPressurePSI]); // Loss of boiler heat due to water injection - loss is the diff between steam and water Heat  
                }
                if (Injector2IsOn)
                {
                    ...
                    {
                        Injector2TempFraction = (Injector2Fraction - InjCapMinFactorX[BoilerPressurePSI]) / (1 - InjCapMinFactorX[PSI]); // Find the fraction above minimum value
                    ...
                    BoilerHeatBTU -= elapsedClockSeconds * Injector2Fraction * InjectorFlowRateLBpS * (BoilerWaterHeatBTUpLB - WaterHeatPSItoBTUpLB[Injector2WaterTempPressurePSI]); // Loss of boiler heat due to water injection - loss is the diff between steam and water Heat      
                    ...
                    BoilerHeatOutBTUpS += Injector2Fraction * InjectorFlowRateLBpS * (BoilerWaterHeatBTUpLB - WaterHeatPSItoBTUpLB[Injector2WaterTempPressurePSI]); // Loss of boiler heat due to water injection - loss is the diff between steam and water Heat 
                }
            }
        }
        ...
        private void UpdateFiring(float absSpeedMpS)
        {
            ...
            #endregion
            // Determine Heat Ratio
            HeatRatio = MathHelper.Clamp((BoilerHeatOutBTUpS / BoilerHeatInBTUpS), 0.1f, 1.5f);  // Factor to determine how hard to drive burn rate, based on steam gen & usage rate, in AI mode only
        }
    ...
    // +++++++++++++++ Main Simulation - End +++++++++++++++++++++
        ...
        public override string GetDebugStatus()
        {
            ...
            status.AppendFormat("Thermal:\tIn\t{0:N0} BTU\t\tout\t{1:N0} BTUt\t\tHR\t{2:N2}\n",
                BoilerHeatInBTUpS,
