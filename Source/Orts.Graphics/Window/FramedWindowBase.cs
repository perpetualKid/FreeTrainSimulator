...
{
    public abstract class FramedWindowBase : WindowBase
    {
        ...
        protected FramedWindowBase(WindowManager owner, string caption, Point relativeLocation, Point size, Catalog catalog = null) : 
            base(owner, caption, catalog)
        {
            ...
            if (size.X < 0)
            {
                ...
                size.X = owner.ClientBounds.Width * -size.X / 100;
            }
            if (size.Y < 0)
            {
                if (size.X < -100)
                    throw new ArgumentOutOfRangeException(nameof(size), "Relative window size must be defined in range between -0 to -100 (% of game window size)");
                size.Y = owner.ClientBounds.Height * -size.Y / 100;
            }
