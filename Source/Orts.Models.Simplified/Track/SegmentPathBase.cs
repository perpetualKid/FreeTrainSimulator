...
namespace Orts.Models.Simplified.Track
{
    ...
    public abstract class SegmentPathBase<T>: VectorPrimitive where T : SegmentBase
    {
        private readonly PointD midPoint;

        internal protected readonly List<T> pathSegments = new List<T>();
...
#pragma warning disable CA2214 // Do not call overridable methods in constructors
        protected SegmentPathBase(in PointD start, int startTrackNodeIndex, in PointD end, int endTrackNodeIndex, Dictionary<int, List<SegmentBase>> sourceElements):
            base(start, end)
        {
            midPoint = Location + (Vector - Location) / 2.0;
